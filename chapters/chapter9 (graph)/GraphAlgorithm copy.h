...
#include "../chapter6/BinaryHeap.h"
#include "../chapter8/unionFindSet.h"

class GraphAlgorithm
{
private:
    /// dfsSearchEulerCircuitHelper
    void dfsSearchEulerHelper(const VectorGraph &graph, const int startVertex,
    vector<int> &vertexVector, set<pair<int, int> > &edgeSet)
    {
        for (auto &edge : graph[startVertex])
        {
            ...
            {
                edgeSet.insert(pair);
                dfsSearchEulerHelper(graph, edge.first, vertexVector, edgeSet);
            }
        }
        vertexVector.push_back(startVertex);
    }

    void minDistanceSingalSourceUnWeightedAllPathHelper(const vector<vector<int> >& pvs,
    const int startVertex, const int vertex, vector<vector<int> >& paths)
    {
        if(vertex==startVertex)
            paths.push_back(vector<int>{vertex});
        else
            for (auto pv: pvs[vertex])
            {
                minDistanceSingalSourceUnWeightedAllPathHelper(pvs,startVertex, pv, paths);
                for (auto& path: paths)
...
public:
    ...
    */
    ...
    int startVertex, int endVertex)
    {
        ...
        minDistanceSingalSourceUnWeightedAllPathHelper(pvs, startVertex, endVertex, paths);
        for (auto& path: paths)
        {
            for (auto v: path)
                printf("%d ", v);
    ...
    */
    ...
    /// Minimum Spanning Tree(有权无向图最小生成树算法系列)
    /*
    ...
    */
    ...
    /// Euler Circuit and Euler Path Algorithm(欧拉回路与欧拉路径问题)
    /*
    ...
    */
    ...
    void dfsSearchEulerCircuit(const VectorGraph &graph)
    {
        ...
        dfsSearchEulerHelper(graph, 1, vertexVector, edgeSet);
    ...
    void dfsSearchEulerPath(const VectorGraph& graph)
    {
        ...
        vector<int> vertexVector;
        set<pair<int, int> > edgeSet;
        dfsSearchEulerHelper(graph, startVertex, vertexVector, edgeSet);
        for (int i = vertexVector.size()-1; i >= 0; i--)
        {
            cout<<vertexVector[i]<<" ";
        }
